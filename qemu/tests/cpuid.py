...
def run_cpuid(test, params, env):
    ...
    class check_cpuid_dump(MiniSubtest):
        """
        ...
        def test(self):
            ...
            for k in reference.keys():
                ...
                for d in diffs:
                    ...
                    info("Non-matching bit: CPUID[0x%x,0x%x].%s[%d]: found %s instead of %s", in_eax, in_ecx, reg, bit, vout, vreference)
                    if (in_eax,) in whitelist \
                        or (in_eax, in_ecx) in whitelist \
                        or (in_eax, in_ecx, reg) in whitelist \
                        or (in_eax, in_ecx, reg, bit) in whitelist:
                            info("(mismatch is whitelisted)")
                            continue
                    ok = False
            if not ok:
